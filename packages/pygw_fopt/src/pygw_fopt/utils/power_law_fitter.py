...
class PowerLawFitter:
    ...
    def __init__(self, x_data, y_data, fitting_powers, weights=None, x_range=None, 
                 small_x_range=None, large_x_range=None, match_condition=False):
        """
        ...
        self.x_range = x_range if x_range is not None else (np.min(x_data), np.max(x_data))
    ...
    def _validate_inputs(self):
        ...
        if self.small_x_range is not None:
            if not (self.small_x_range[0] <= self.small_x_range[1] and self.small_x_range[0] > 0):
                raise ValueError("small_x_range must be valid and positive")
        if self.large_x_range is not None:
            if not (self.large_x_range[0] <= self.large_x_range[1] and self.large_x_range[0] > 0):
                raise ValueError("large_x_range must be valid and positive")

    def _fit_small_range(self):
        """Fit y = a * x^b in the small x range."""
        mask_small = (self.x_data >= self.small_x_range[0]) & (self.x_data <= self.small_x_range[1])
        ...
        try:
            popt_small, pcov_small = curve_fit(
                model_small, x_small, y_small,
                p0=[1.0, 1.0], bounds=(0, np.inf)
            )
        ...
        self.fit_results['small'] = {'popt': popt_small, 'pcov': pcov_small, 'model': fitted_model_small}
        self.fit_info['small_fit'] = f"Small x fit: y = {popt_small[0]:.4e} x^{popt_small[1]:.4e}"

    def _fit_large_range(self):
        """Fit sum(A_i * x^powers[i]) in the large x range with all A_i > 0."""
        # Filter data
        if self.large_x_range is not None:
            mask_large = (self.x_data >= self.large_x_range[0]) & (self.x_data <= self.large_x_range[1])
            ...
            weights_large = self.weights[mask_large] if self.weights is not None else None
        else:
            x_large = self.x_data
            y_large = self.y_data
        ...
        if self.match_condition:
            if self.fit_results['small'] is None:
                raise ValueError("match_condition=True requires a successful small x fit")
            ...
            if len(self.fitting_powers) == 1:
                ...
                self.fit_results['large'] = {'popt': popt_large, 'pcov': pcov_large, 'model': fitted_model_large}
                terms = [f"{popt_large[i]:.4e} x^{self.fitting_powers[i]}" for i in range(len(self.fitting_powers))]
                self.fit_info['large_fit'] = f"Large x fit: y = {' + '.join(terms)}"
                self.fit_info['sum_coeffs'] = f"Sum of large x coefficients: {sum(popt_large):.4e}, expected: {a_small:.4e}"
                if self.fit_results['small'] is not None:
                    ...
                    self.fit_info['at_x1'] = f"At x=1: small fit y = {y_small_at_x1:.4e}, large fit y = {y_large_at_x1:.4e}"
            else:
                ...
                try:
                    popt_b, pcov_b = curve_fit(
                        ...
                        p0=p0_large, sigma=1.0 / np.sqrt(weights_large),
                        absolute_sigma=True, bounds=(bounds_lower, bounds_upper),
                        maxfev=10000
                    )
                    # Compute a_params
                    a_params = []
                    ...
                    def fitted_model_large(x):
                        return sum(a * x**p for a, p in zip(popt_large, self.fitting_powers))
                    self.fit_results['large'] = {'popt': popt_large, 'pcov': pcov_large, 'model': fitted_model_large}
                    terms = [f"{popt_large[i]:.4e} x^{self.fitting_powers[i]}" for i in range(len(self.fitting_powers))]
                    self.fit_info['large_fit'] = f"Large x fit: y = {' + '.join(terms)}"
                    self.fit_info['sum_coeffs'] = f"Sum of large x coefficients: {sum(popt_large):.4e}, expected: {a_small:.4e}"
                    ...
                    raise RuntimeError(f"Large x range fitting failed with constraint: {e}")
        else:
            def model_large(x, *params):
                if np.any(np.array(params) <= 0):
                    raise ValueError("Negative coefficient detected")
                return sum(param * x**power for param, power in zip(params, self.fitting_powers))
    ...
    def get_normalized_coeffs_with_cov(self):
        ...
        if self.match_condition:
            for i in range(n - 1):
                for j in range(n - 1):
                    if i == j:
                        pcov_coeffs[i,i] = (1 / a_small**2 * pcov_large[i,i]) + (popt_large[i]**2 / a_small**4 * var_a_small)
                    else:
    ...
    def get_rss(self):
        ...
        mask_large = (self.x_data >= self.large_x_range[0]) & (self.x_data <= self.large_x_range[1]) if self.large_x_range is not None else np.ones_like(self.x_data, dtype=bool)
        x_large = self.x_data[mask_large]
        y_large = self.y_data[mask_large]
    ...
    def get_fit_metrics(self):
        ...
        rss = self.get_rss()
        tss = self.get_tss()
        n = np.sum((self.x_data >= self.large_x_range[0]) & (self.x_data <= self.large_x_range[1]) if self.large_x_range is not None else np.ones_like(self.x_data, dtype=bool))
        k = len(self.fitting_powers) - 1 if self.match_condition else len(self.fitting_powers)
    ...
    def plot_model_comparison(self, fitting_powers_list, metric='bic'):
        ...
        bars = ax.bar(range(len(n_params)), metric_values, tick_label=labels, color='tab:blue', alpha=0.7)
        
        # Highlight best model
        best_idx = labels.index(f"[{', '.join(map(str, comparison['best_fitting_powers']))}]")
        bars[best_idx].set_color('tab:orange')
        bars[best_idx].set_label(f'Best model ({metric.upper()} = {comparison["best_metrics"][metric]:.2f})')
        
        # Plot settings
        ax.set_xlabel('Fitting Powers')
        ax.set_ylabel(metric.upper())
        ax.set_title(f'Model Comparison by {metric.upper()} (Best: {", ".join(map(str, comparison["best_fitting_powers"]))})')
    ...
    def plot(self, small_plot_to_x1=True, large_plot_from_x1=True):
        ...
        """
        fig, ax = plt.subplots()
        ax.plot(self.x_data, self.y_data, color="red", label="Data", marker="o", ms=2, linestyle="none")
        ...
        large_plot_range = self.large_x_range if self.large_x_range is not None else self.x_range
    ...
    def get_models(self):
        ...
        """
        return {
            'small': self.fit_results['small']['model'] if self.fit_results['small'] is not None else None,
    ...
    def get_formatted_string(self):
        ...
        formatted_string = "[" + ", ".join([f"{x}" for x in (-self.fitting_powers - 2)]) + "]"
        formatted_string += r"$,~ a_\xi = $"
        formatted_string += "[" + ", ".join([f"{x:.2e}" for x in fitting_coefficients]) + "]"
