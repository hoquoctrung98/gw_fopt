...
class SurfaceTensionFitter:
    def __init__(self, df_surface_tension, filter_dict, d_values, xi_powers,
                 nonlinear_cutoff_ratio, x_fit_right, Lw):
        """
        ...
        """
        if 'd' in filter_dict:
            raise ValueError("filter_dict must not contain the key 'd' — it is varied separately.")
    ...
    def _fit_func(self, x, a_0, a_mid):
        ...
        return a_0 / (x**(self.xi_powers[0] + 2)) + a_mid / (x**(self.xi_powers[1] + 2))
    ...
    def fit_data(self, minimization_method='COBYQA'):
        ...
        for d in self.d_values:
            ...
            # Rough estimate of σ₀ from small-x plateau using PowerLawFitter logic
            # (replicating the original behavior without full dependency)
        ...
        for _, row in df_res.iterrows():
            f = interp1d(row['x_data'], row['y_data'] / row['sigma0'],
        ...
        x_fit_left = max(1 + 2 * self.nonlinear_cutoff_ratio * self.Lw / d for d in self.d_values)
        ...
        res = minimize(self._chi2, x0=[0.2, 0.1], args=(data,),
                       constraints=constraints, method=minimization_method,
    ...
    def compute_confidence_region(self, n_points=200):
        ...
        data = np.column_stack([self.x_fit, self.mean_y_fit, self.err_y_fit])
        chi2_grid = np.vectorize(lambda a0, amid: self._chi2([a0, amid], data))(A0, Amid)
    ...
    def plot_surface_tension(self, fig, ax):
        ...
        for res in self.results:
            ax.plot(res['x_data'], res['y_data'] / res['sigma0'],
                    'o', ms=2, alpha=0.6, label=f"d = {res['d']}")

        ax.plot(self.x_common, self.mean_y, 'k-', lw=2, label=r'Mean $\sigma/\sigma_0$')
        ax.fill_between(self.x_common,
        ...
        ax.fill_between(self.x_fit, self.mean_y.min(), model_fit, color='red', alpha=0.3, zorder=5)
        ...
        ax.set_title(
            ...
            "\n" + rf"$\xi = [{', '.join(f'{x:}' for x in self.xi_powers[:-1])}$" + r"$, \infty]$" + \
    ...
    def plot_confidence_region(self, n_points=200):
        ...
        ax.contour(A0, Amid, self.delta_chi2_ndof, levels=[1.0], colors='black', linewidths=2)

        ax.plot(self.a_0, self.a_mid, 'o', color='white', markeredgecolor='black', markersize=8)

        # Forbidden region a_0 + a_mid > 1
        ax.contourf(A0, Amid, A0 + Amid, levels=[1, 2], colors='gray', alpha=0.3, hatches=['//'])

        ax.set_xlabel(r'$a_0$')
        ax.set_ylabel(r'$a_{\rm mid}$')
        ax.set_title(rf"Confidence region – $a_0 = {self.a_0:.3f} \pm {self.err_a_0:.3f}$, "
