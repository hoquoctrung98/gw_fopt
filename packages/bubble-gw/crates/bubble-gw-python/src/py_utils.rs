...
///
/// This function generates a sequence of samples between `start` and `stop`
/// using the specified sampling type. The samples are returned as a `NumPy`
/// array for efficient numerical computations.
///
/// # Arguments
/// * `start` (float): The start of the sampling range (must be less than
///   `stop`).
/// * `stop` (float): The end of the sampling range.
/// * `n_sample` (int): Number of intervals (produces `n_sample + 1` points for
///   `n_iter = 0`).
/// * `n_grid` (int): Number of grid points for iterative sampling (must be >= 2
///   for `n_iter > 0`).
/// * `n_iter` (int): Number of iterations for refined sampling (non-negative).
/// * `sample_type` (str): Type of sampling ("uniform", "linear", "log", "exp").
/// * `base` (float, optional): Base for logarithmic or exponential sampling
...
/// For `n_iter > 0`: returns **only the new interior points** introduced at
/// this refinement level (i.e., excludes points from coarser grids).
///
...
/// Performs sampling over multiple iterations based on the specified
/// parameters.
///
/// This function generates samples for iterations from 0 to `n_iter` and
/// concatenates them into a single `NumPy` array. Useful for generating samples
...
///
/// # Returns
/// Concatenates samples from `n_iter = 0..=n_iter`, producing a full nested
/// dyadic grid with no duplicate points. Ideal for hierarchical training or
/// multi-resolution analysis. # Raises
/// * `ValueError`: If `start >= stop`, `base <= 0` for logarithmic/exponential
///   sampling, `n_sample == 0`, `n_grid < 2` for `n_iter > 0`, or if
///   `sample_type` is invalid.
///
/// # Examples
/// ```python
