...
//                     let points = lattice.sample_points(*n_points, &mut
// self.rng);                     self.points_outside_bubbles = Some(points);
//                 }
//
//                 let points_outside_bubbles =
// self.points_outside_bubbles.as_mut().unwrap();                 let
// all_bubbles: Vec<&Vector4<f64>> = bubbles_interior
// .spacetime                     .iter()
...
//
//                 let fraction = points_outside_bubbles.len() as f64 /
// *n_points as f64;                 (fraction * lattice.volume()).max(0.0)
//             },
//         }
//     }
//
...
//             let candidate_pts = lattice.sample_points(batch_size, &mut
// self.rng);
//
...
// }
//
// impl<L: GeneralLatticeProperties> NucleationStrategy<L> for
// FixedRateNucleation {     fn nucleate(
//         &mut self,
//         lattice: &L,
...
//                 self.volume_remaining(lattice, t, &bubbles_interior,
// &bubbles_exterior);
//
...
// volume_remaining).max(f64::EPSILON);             if !dt.is_finite() || dt <=
// 0.0 || dt > 1.0 {                 break;
//             }
//
//             let new_t = t + dt;
...
//             if x <= self.d_p0 {
//                 if let Some(new_bubbles) =
// self.sample_points_outside_bubbles(                     lattice,
//                     new_t,
//                     &bubbles_interior,
//                     &bubbles_exterior,
//                     1,
//                 ) {
//                     if let Some(bubble_new) =
// new_bubbles.spacetime.into_iter().next() {
// bubbles_interior.spacetime.push(bubble_new);                         let
// dummy_interior = Bubbles::new(vec![bubble_new]);                         let
// exterior_bubbles =
