    ...
    F: Fn(f64) -> f64,
{
    ...
    println!("Trapezoid result: {:.6}, Error: {:.6}", trap_result, (trap_result - expected[0]).abs());
    println!("Simpson result: {:.6}, Error: {:.6}", simp_result, (simp_result - expected[0]).abs());

    assert!(
        trap_result.is_close(&expected[0], abs_tol, rel_tol).is_ok(),
        "Slice trapezoid result differs: {}",
        trap_result.is_close(&expected[0], abs_tol, rel_tol).unwrap_err()
    );
    assert!(
        simp_result.is_close(&expected[0], abs_tol, rel_tol).is_ok(),
        "Slice Simpson result differs: {}",
        simp_result.is_close(&expected[0], abs_tol, rel_tol).unwrap_err()
    );
}
    ...
    F: Fn(f64) -> f64,
{
    ...
    println!("Trapezoid result: {:.6}, Error: {:.6}", trap_scalar, (trap_scalar - expected[0]).abs());
    println!("Simpson result: {:.6}, Error: {:.6}", simp_scalar, (simp_scalar - expected[0]).abs());

    assert!(
        trap_scalar.is_close(&expected[0], abs_tol, rel_tol).is_ok(),
        "Array1 trapezoid result differs: {}",
        trap_scalar.is_close(&expected[0], abs_tol, rel_tol).unwrap_err()
    );
    assert!(
        simp_scalar.is_close(&expected[0], abs_tol, rel_tol).is_ok(),
        "Array1 Simpson result differs: {}",
        simp_scalar.is_close(&expected[0], abs_tol, rel_tol).unwrap_err()
    );
}
    ...
    F: Fn(f64) -> f64,
{
    ...
    assert!(
        ...
        trap_result2.is_close(&expected, abs_tol, rel_tol).unwrap_err()
    );
    assert!(
        simp_result2.is_close(&expected, abs_tol, rel_tol).is_ok(),
        "Array2 Simpson results differ: {}",
        simp_result2.is_close(&expected, abs_tol, rel_tol).unwrap_err()
    );
}
    ...
    F: Fn(f64) -> f64,
{
    ...
    println!("\nTesting slice integration with x = range(0., 11., 1.), y = integrand(x) (n={}):", n);
    let x_slice = x.as_slice().unwrap();
    let simp_result = y.as_slice().unwrap().simpson(Some(x_slice), None, None).unwrap();
    println!("Simpson result: {:.6}, Error: {:.6}", simp_result, (simp_result - expected[0]).abs());
    assert!(
        simp_result.is_close(&expected[0], abs_tol, rel_tol).is_ok(),
        "Slice Simpson result differs: {}",
    ...
    println!("\nTesting Array1 integration with x = range(0., 11., 1.), y = integrand(x) (n={}):", n);
...
fn test_complex_slice_integration<F>(n: usize, integrand: F, expected: &Complex<f64>, abs_tol: f64, rel_tol: f64)
where
    F: Fn(f64) -> Complex<f64>,
{
    ...
    let trap_result = (&y as &[Complex<f64>]).trapezoid(Some(&x), None, None).unwrap();
    let simp_result = (&y as &[Complex<f64>]).simpson(Some(&x), None, None).unwrap();
    ...
    assert!(
        ...
        trap_result.is_close(expected, abs_tol, rel_tol).unwrap_err()
    );
    assert!(
        simp_result.is_close(expected, abs_tol, rel_tol).is_ok(),
        "Complex slice Simpson result differs: {}",
        simp_result.is_close(expected, abs_tol, rel_tol).unwrap_err()
    );
}

// Test function for complex Array1 integration
fn test_complex_array1_integration<F>(n: usize, integrand: F, expected: &Complex<f64>, abs_tol: f64, rel_tol: f64)
where
    F: Fn(f64) -> Complex<f64>,
{
    let x: Vec<f64> = (0..n).map(|i| i as f64 / (n - 1) as f64).collect();
    ...
    assert!(
        ...
        trap_scalar.is_close(expected, abs_tol, rel_tol).unwrap_err()
    );
    assert!(
        simp_scalar.is_close(expected, abs_tol, rel_tol).is_ok(),
        "Complex Array1 Simpson result differs: {}",
        simp_scalar.is_close(expected, abs_tol, rel_tol).unwrap_err()
    );
}
