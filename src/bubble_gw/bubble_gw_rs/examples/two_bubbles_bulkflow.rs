...
fn main() -> Result<(), BulkFlowError> {
    ...
    let mut bulk_flow = BulkFlow::new(bubbles_interior, bubbles_exterior)?;
    ...
    let collision_status =
        bulk_flow.compute_collision_status(a_idx, t, first_bubble, delta_tab_grid.view())?;
    let delta_ta = t - bulk_flow.bubbles_interior()[[a_idx, 0]];

    let (b_plus, b_minus) = bulk_flow.compute_b_integral(
        25,
        collision_status.view(),
        delta_tab_grid.view(),
        delta_ta,
    )?;
