...
class GwTwoBubblesVisualizer:
    ...
    def plot_gw_averaged(self, fig, ax, **kwargs_plot):
        ...
        ax.set_ylabel(
            r"$\widehat{\Omega}_\text{GW} = \dfrac{dE_\text{GW}}{d\log \omega} = 2 \displaystyle\int_0^1 d \cos \theta_k \dfrac{dE_\text{GW}}{d \log \omega d \cos \theta_k}$"
        )
    ...
    def plot_all_gw_angular(self, fig, ax):
        ...
        for k, cos_thetak in enumerate(cos_thetak_arr):
            ...
            ax.plot(
                w_arr,
                dE_dlogw_dcosthetak,
                label=rf"$\cos \theta_k = {cos_thetak:.2f}$",
                color=colors[k],
            )
        ax.set_xscale("log")
        ax.set_yscale("log")
        ...
        ax.yaxis.set_minor_locator(ticker.LogLocator(numticks=999, subs="auto"))
        ax.grid(True, which="both", alpha=0.5)
        fig.legend(bbox_to_anchor=(0.95, 0.5), loc="center left", borderaxespad=0.0)
        return fig, ax
    ...
    def plot_one_gw_angular(self, fig, ax, cos_thetak, **kwargs_plot):
        ...
        ax.plot(
            w_arr,
            dE_dlogw_dcosthetak,
            label=rf"$\cos \theta_k = {cos_thetak_arr[closest_idx]:.2f}$, $(1+1)D$ simulation",
            **kwargs_plot,
        )
        ax.set_xscale("log")
        ax.set_yscale("log")
