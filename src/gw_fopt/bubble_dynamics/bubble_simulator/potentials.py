...
class U1Potential(GenericPotential):
    ...
    def V0(self, X):
        ...
        rval = (
            1
            + kappa * phi2 / v**2
            + (phi2**2 / v**4) * ((kappa + 2) * np.log(phi2 / v**2) - (kappa + 1))
        )
    ...
    def plot_potential(self, phi_range, fixed_phi_im=0.0, num_points=1000):
        ...
        ax.plot(
            ...
            V,
            "b-",
            label="$V_0(\\phi_{\\text{re}}, \\phi_{\\text{im}}="
            + f"{fixed_phi_im}"
            + ")$",
        )
        ...
        if len(maxima_idx) > 0:
            ax.scatter(
                phi_re[maxima_idx], V[maxima_idx], color="red", label="Maxima", zorder=5
            )
        ...
        delta_V, kappa, v = self.params
        ax.set_title(
            rf"Potential with $\Delta V = {delta_V}$, $\\kappa = {kappa}$, $v = {v}$, $\\phi_{{\\text{{im}}}} = {fixed_phi_im}$"
        )
...
class QuarticPotential(GenericPotential):
    ...
    def plot_potential(self, fig, ax, phi_range, num_points=1000):
        ...
        if len(minima_idx) > 0:
            ax.scatter(
                phi[minima_idx], V[minima_idx], color="green", label="Minima", zorder=5
            )
        if len(maxima_idx) > 0:
            ax.scatter(
                phi[maxima_idx], V[maxima_idx], color="red", label="Maxima", zorder=5
            )
