...
############################################################
# YAML-driven patched simulation parameters.
DEFAULT_OUTPUT_DIR = Path("tests/data/two_bubbles_code_v1_0_1")
SUPPORTED_BUBBLE_TYPES = {"half", "one", "whole"}
...
def field_evolution_h5_path(config):
...
def copied_config_path(config, config_path):
    directory = output_dir(config)
...
def write_metadata(h5, config, raw_yaml):
...
def write_field_evolution_hdf5(path, config, raw_yaml, simulation):
    path.parent.mkdir(parents=True, exist_ok=True)
    with h5py.File(path, "w") as h5:
        write_metadata(h5, config, raw_yaml)

        sim_group = h5.create_group("simulation")
        for key in [
            "lambda_bar",
            "gamma",
            "bubble_type",
            ...
            sim_group.attrs[key] = simulation[key]
        write_dataset(sim_group, "z", simulation["z"])
        write_dataset(sim_group, "s", simulation["s"])
        write_dataset(sim_group, "phi_region1", simulation["phi_region1"])
        write_dataset(sim_group, "phi_mid", simulation["phi_mid"])
        write_dataset(sim_group, "energy_density", simulation["energy_density"])
        write_dataset(sim_group, "r_info", simulation["r_info"])


parser = argparse.ArgumentParser(
    description="Patched two_bubbles_code field evolution to HDF5."
)
...
output_h5 = field_evolution_h5_path(config)
if output_h5.exists() and not bool(run_config.get("overwrite", False)):
    raise FileExistsError(f"{output_h5} exists; set run.overwrite=true to replace it.")

lambdabar = float(simulation_config["lambda_bar"])
gamma_value = simulation_config["gamma"]
gamma = float(gamma_value) if "." in str(gamma_value) else int(gamma_value)
bubble_type = str(simulation_config.get("bubble_type", "half"))
if bubble_type not in SUPPORTED_BUBBLE_TYPES:
    supported = ", ".join(sorted(SUPPORTED_BUBBLE_TYPES))
    raise ValueError(
        f"Unsupported simulation.bubble_type={bubble_type!r}; expected one of: {supported}."
    )

ratio_smax_d = float(simulation_config.get("ratio_smax_d", 1.2))
dz_times = simulation_config.get("dz_times")
ds_config = simulation_config.get("ds")
dz_mult = 1 if dz_times is None else float(dz_times)
set_ds = ds_config is not None
if set_ds:
    ds_hard = float(ds_config)

where_to_save_data = str(output_dir(config)) + os.sep
Path(where_to_save_data).mkdir(parents=True, exist_ok=True)
...
def default_run(ds, name):
    ...
    ds_saved = ds * how_often_ds
    s_saved = np.linspace(0.0, (len(phi_to_save) - 1) * ds_saved, len(phi_to_save))
    simulation = {
        "lambda_bar": float(lambdabar),
        "gamma": gamma,
        "bubble_type": name,
        "ratio_smax_d": float(ratio_smax_d),
        "saved_ratio_smax_d": float(s_saved[-1] / d),
        "d": float(d),
        "ds_original": float(ds),
        "ds_saved": float(ds_saved),
        "n_z_info": int(n_z),
        "how_often_ds": int(how_often_ds),
        "r_info": np.asarray(r_info, dtype=np.float64),
        "z": np.asarray(z, dtype=np.float64),
        "s": s_saved,
        "phi_region1": np.asarray(phi_to_save, dtype=np.float64),
        "phi_mid": np.asarray(phi_mid, dtype=np.float64),
        "energy_density": np.asarray(energy_density, dtype=np.float64),
        "dz": float(abs(z[1] - z[0])),
    }
    write_field_evolution_hdf5(output_h5, config, raw_yaml, simulation)
