...
+ `bubble_gw` compute the GW spectrum via different modules:
  ...
  +`bubble_gw.two_bubbles` compute the exact GW spectrum of two bubbles system by taking the input of fields evolution on the $1+1$ lattice computed from `bubbles_dynamics` and numerically compute the GW spectrum via  the fourier transformed stress-energy tensor.
The core computation of exact two bubbles spectrum is a reimplementation of the [two_bubbles_code-v1.0.1](https://zenodo.org/records/5127538.), leveraging Rust for improved computational efficiency and PyO3 for Python integration.
...
2. Install `uv`, a Python package and project manager
...
+ [two bubbles](./docs/two_bubbles.md): Here we use `bubbles_dynamics` to solve the equation of motion on $1+1$D lattice of a quartic potential and plugging this to `bubbles_dynamics.two_bubbles` to compute the exact GW spectrum. 
