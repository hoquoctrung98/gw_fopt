...
=== "Python"
    ...
    An executable Python workflow is available at [two_bubbles.py](./examples/two_bubbles.py).

    ```bash
    cd docs/examples
    uv run python two_bubbles.py
    ```

=== "Rust"

    The native exact-spectrum implementation is exposed from the `bubble_gw::two_bubbles` module.
    Build the crate documentation to inspect the Rust API:

    ```bash
    cd rust
    cargo doc -p bubble_gw --no-deps --open
    ```
