...
history_interval = (
    10  # number of time steps before saving, smaller means better time resolution
)
solver = PDEBubbleSolver(
    phi1_initial=phi_initial, z_grid=z_grid, ds=ds, dz=dz, potential=potential, d=d
)
phi_evolution = solver.evolve(
    smax, history_interval=history_interval
...
fig.savefig(f"./figures/evolution_field.png", bbox_inches="tight", facecolor="white")

fig, ax = analyzer.plot_gradient_energy_density(plot_boundaries=True, cutoff=1e-4)
fig.savefig(
    f"./figures/evolution_gradient_energy_density.png",
    bbox_inches="tight",
    facecolor="white",
)
...
fig.savefig(f"./figures/surface_tension.png", bbox_inches="tight", facecolor="white")
...
dE_dlogw_dcosthetak = gw_calc.compute_angular_gw_spectrum(
    w_arr=w_arr, cos_thetak_arr=[0.0]
)
